...
def valid_test(beta1, beta2, beta3, beta4, model, dataloader, batch_size, path, epoch, length_v, dataset):
    ...
    return mean_loss, mean_loss1, mean_loss2, mean_loss3, mean_loss4, mean_loss5, results, mse
   
def closs(poc_att, pro_att):
    tau = 0.07
    score = F.cosine_similarity(poc_att, pro_att, dim=-1)
    score = score / tau  
    is_positive = torch.diag(torch.ones(len(poc_att), dtype=torch.bool))
...
def process_data(datasets, datacsv_path, label):
    ...
    if label == "new_drug": 
        ...
        random.seed(1234) 
        random.shuffle(did)
        train_data = did[0: int(len(did)*0.78)]
        valid_data = did[int(len(did)*0.78):int(len(did))]
        for i in datasets:
        ...
        random.seed(1234)
        random.shuffle(pid)
        train_data = pid[0: int(len(pid)*0.84)]
        valid_data = pid[int(len(pid)*0.84):int(len(pid))]
        for i in datasets:
        ...
        random.shuffle(did)
        random.shuffle(pid)
        pid_valid = pid[0: int(len(pid)*0.33)]
        pid_train = pid[int(len(pid)*0.33): int(len(pid))]
        did_valid = did[0: int(len(did)*0.33)]
        did_train = did[int(len(did)*0.33): int(len(did))]
...
def main(params):
    ...
    train_dataset, valid_dataset = process_data(datasets, datacsv_path, label=args.label)
    ...
    for epoch in range(args.EPOCHS):
        ...
        if valid_mse < best_mse:
            best_mse = valid_mse
            best_mse_results = valid_results
